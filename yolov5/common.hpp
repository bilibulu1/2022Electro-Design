...
int a = set_uart_config(UART_fd, 9600, 8, 'N', 1);
cv::Rect get_rect(cv::Mat& img, float bbox[4]) {
    ...
    if (r_h > r_w) {
        ...
        center_x=(l+r)/2;
        unsigned char x_high=center_x/255;
        unsigned char x_low=center_x%255;
        center_y=(t+b)/2;
        unsigned char y_high=center_y/255;
        unsigned char y_low=center_y%255; 
        unsigned char str[8];
        str[0]=255;
        str[1]=254;
        str[2]=253;
